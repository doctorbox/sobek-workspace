...
function BlockCard({
  ...
  return (
    <div className="flex overflow-hidden w-full bg-white rounded-lg border border-solid border-zinc-200 shadow-[0px_1px_2px_rgba(0,0,0,0.06)]">
      <div className="w-2 self-stretch bg-purple-200" />
      <div className="flex flex-col flex-1">
        <div className="flex-1 p-10 border-b border-solid border-b-zinc-200 max-md:p-5">
          ...
          <a
            href={moreInfoURL}
            className="mt-6 font-medium underline text-zinc-950 inline-block"
          >
            Show more
          </a>
        </div>
        <div className="flex px-10 py-8 max-md:p-5">
          <button
            className="px-8 py-2.5 text-sm font-medium text-blue-900 rounded-md border border-current border-solid cursor-pointer max-md:px-5 max-md:py-2.5 max-md:w-full"
            onClick={() =>
              router.push(`${router.asPath}/${blockNumber}/score-source`)
            }
          >
