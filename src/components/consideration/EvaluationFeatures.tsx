...
export const EvaluationFeatures: React.FC<EvaluationFeaturesProps> = ({
  ...
  return (
    <div className="flex flex-wrap flex-col gap-10 items-start p-5 bg-white max-w-[920px] max-sm:flex-col max-sm:gap-8 max-sm:p-4">
        ...
        {features.map((feature, index) => (
          <React.Fragment key={index}>
            ...
            {index < features.length - 1 && (
              <div className="hidden mx-5 my-0 border border-solid border-zinc-200 h-[152px]" />
            )}
          </React.Fragment>
        ))}
      </div>
